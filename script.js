...
function play() {
  ...
  do {
    var guess = prompt('Þú átt að giska á tölu á bilinu 0 til 100')
    if (guess === null) {
      break;
    }
    var parseG = parseGuess(guess);
    alert(getResponse(parseG, random));
    if (parseG === random) {
      games.push(userGuess);
    } else {
      userGuess++;
    }
  } while (parseG !== random);
...
function getResults() {
  ...
  var av = calculateAverage();
  var res = 'Þú spilaðir ' + games.length + ' leiki.' + '\n' + 'Meðalfjöldi ágiskana var ' + av;
  return res;
}
...
function calculateAverage() {
  ...
  return (a / games.length).toFixed(2);
}
...
function getResponse(guess, correct) {
  ...
  var gap = Math.abs(correct - guess);
  if (gap < 5) {
    return 'Mjög nálægt';
  }
  if (gap < 10) {
